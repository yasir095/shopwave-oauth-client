...
//    postData = (typeof req.postBody !== 'undefined') ? req.postBody : {};
    ...
    makeWebRequest(requestObject, function(response)
    {
        ...
        if(!requestProcessed)
        {
            ...
            {
                /**
                 * Supply the regex in the form below:
                 * changes will be applied to whole json.
                 * req.dataOverride {searchString: "/http:\/\/static\.merchantstack\.com/gi", replaceWith: "your new string or url."}
                 */

                if(Boolean(req.dataOverride) && response.data.body !== undefined)
                {
                    let searchString = req.dataOverride.searchString;
                    let replaceWith = req.dataOverride.replaceWith;

                    response.data.body = response.data.body.replace(searchString, replaceWith);
                }
